# Hi, I'm Nguyen Minh Quy 👋

I'm a backend developer focused on .NET, SQL Server, Docker, and React.

![C#](https://img.shields.io/badge/C%23-512BD4?style=flat&logo=csharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat&logo=dotnet&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=flat&logo=microsoftsqlserver&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)

## 🚀 Currently working on

I'm building **CommercialNews**, a production-style news platform with ASP.NET Core Web API, SQL Server, Docker, RabbitMQ, React / Next.js, and Google Cloud VM deployment.

## 🛠️ Tools I use

**Backend:** C#, .NET 8, ASP.NET Core Web API  
**Database:** SQL Server, T-SQL, Stored Procedures  
**DevOps:** Docker, Docker Compose, Nginx, GitHub Actions, Google Cloud  
**Frontend:** React, Next.js, TypeScript, Tailwind CSS  
**Tools:** Git, Linux, Postman, Swagger, VS Code

## 📌 Featured project

**CommercialNews**  
A full-stack news management platform built with .NET, SQL Server, React, Docker, RabbitMQ, and Google Cloud.

- Backend: https://github.com/minhquy1805/CommercialNews
- Frontend: https://github.com/minhquy1805/commercialnews-web
- Deployment: https://github.com/minhquy1805/commercialnews-deploy
- Admin Demo: https://admin.minhquy.dev
- Portfolio: https://portfolio.minhquy.dev

## 📫 Contact

- Email: minhquy073@gmail.com
- LinkedIn: https://www.linkedin.com/in/quy-nguyen-823109344
- Portfolio: https://portfolio.minhquy.dev